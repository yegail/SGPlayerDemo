...
@property (nonatomic, strong) UIButton *startOrStopBarrageBtn;
...
- (void)orientationLeftFullScreen {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        self.topBar.frame = CGRectMake(0, 20, self.keyWindow.bounds.size.width, bottomBaHeight);
    ...
    self.startOrStopBarrageBtn.hidden = NO;
    self.barrageManager.renderView.hidden = NO;
    self.barrageManager.renderView.frame = CGRectMake(0.0, 0.0, self.keyWindow.bounds.size.width, self.keyWindow.bounds.size.height/3);
    if ([[NSUserDefaults standardUserDefaults] boolForKey:BarrageStatus]) {
        self.startOrStopBarrageBtn.selected = YES;
        if (self.barrageManager.renderView.renderStatus == OCBarrageRenderPaused) {
...
- (void)smallScreen {
    ...
    self.startOrStopBarrageBtn.hidden = YES;
...
- (void)actionStartOrCloseBarrage:(UIButton *)sender {
    sender.selected = !sender.selected;
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setBool:sender.selected forKey:BarrageStatus];
    if (sender.selected) {
        [self.barrageManager start];
        [self performSelector:@selector(addNormalBarrage) withObject:nil afterDelay:0.5];
    }
...
- (void)addNormalBarrage {
    ...
    textDescriptor.text = [NSString stringWithFormat:@"~我是弹幕~"];
    ...
    textDescriptor.fixedSpeed = 20.0;//用fixedSpeed属性设定速度
    ...
    [self performSelector:@selector(addNormalBarrage) withObject:nil afterDelay:0.65];
}
...
- (UIView *)topBar {
    ...
    if(!_topBar) {
        ...
        barrageBtn.translatesAutoresizingMaskIntoConstraints = NO;
        barrageBtn.backgroundColor = [UIColor clearColor];
        [barrageBtn setImage:[UIImage imageNamed:@"danmu_normal"] forState:UIControlStateNormal];
        [barrageBtn setImage:[UIImage imageNamed:@"danmu_selected"] forState:UIControlStateSelected];
        [barrageBtn addTarget:self action:@selector(actionStartOrCloseBarrage:) forControlEvents:UIControlEventTouchUpInside];
        [_topBar addSubview:barrageBtn];
        self.startOrStopBarrageBtn = barrageBtn;
        
        NSLayoutConstraint *barrageBtnRight = [NSLayoutConstraint constraintWithItem:barrageBtn attribute:NSLayoutAttributeRight relatedBy:NSLayoutRelationEqual toItem:_topBar attribute:NSLayoutAttributeRight multiplier:1.0f constant:0];
        NSLayoutConstraint *barrageBtnTop = [NSLayoutConstraint constraintWithItem:barrageBtn attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:_topBar attribute:NSLayoutAttributeTop multiplier:1.0f constant:0];
        NSLayoutConstraint *barrageBtnBottom = [NSLayoutConstraint constraintWithItem:barrageBtn attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:_topBar attribute:NSLayoutAttributeBottom multiplier:1.0f constant:0];
        NSLayoutConstraint *barrageBtnWidth = [NSLayoutConstraint constraintWithItem:barrageBtn attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeWidth multiplier:1.0f constant:60.0f];
        [_topBar addConstraints:@[barrageBtnRight, barrageBtnTop, barrageBtnBottom, barrageBtnWidth]];
    }
    return _topBar;
}
